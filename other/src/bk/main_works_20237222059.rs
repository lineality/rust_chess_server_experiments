    ...
    for request in server.incoming_requests() {
        ...
        if request.method() == &Method::Get {
            ...
            if url_parts.len() >= 3 {
                ...
                // // Load the original game board state
                // let original_board = match load_game_board_state(&game_name) {
                //     Ok(board) => board,
                //     Err(e) => {
                //         eprintln!("Failed to load game state: {}", e);
                //         return;
                ...
                // // Clone the original board to create a board that can be updated with moves
                // let board_after_move = original_board.clone();
                ...
                // if let Err(e) = load_game_board_state(&game_name, &mut *board) {
                //     eprintln!("Failed to load game state: {}", e);
                // }
                ...
                for result in rdr.records() {
                    ...
                    // Original in-place update of the game board
                    //board[from.0][from.1] = ' ';
