    ...
    for request in server.incoming_requests() {
        if request.method() == &Method::Get {
            ...
            if url_parts.len() >= 3 {
                ...
                if let Err(e) = load_game_state(&game_name, &mut *board) {
                    eprintln!("Failed to load game state: {}", e);
                }


                // // Load the game board state from file
                // let loaded_board = match load_game_state(&game_name) {
                //     Ok(board) => board,
                //     Err(e) => {
                //         eprintln!("Failed to load game state: {}", e);
                //         continue;
...
fn load_game_state1(game_name: &str) -> std::io::Result<Board> {
    let dir_path = format!("./games/{}", game_name);
...
fn load_game_state(game_name: &str, board: &mut Board) -> std::io::Result<()> {
