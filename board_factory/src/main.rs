...
fn engine_generate_chessboard_backboard(sandbox_path: &str, orientation_white: bool) -> Result<(), io::Error> {
    ...
    let final_path_in_sandbox = format!("{}/board.png", sandbox_path);
...
fn generate_chessboard_backboard(game_name: &str, orientation_white: bool) -> Result<(), Error> {
    let sandbox_path = format!("games/{}/sandbox", game_name);
    let final_image_path = format!("games/{}", game_name);
    ...
    // Assuming the final image is saved inside sandbox as final_image.png
    let final_image_source = format!("{}/final_image.png", sandbox_path);
    let final_image_destination = format!("{}/final_image.png", final_image_path);
